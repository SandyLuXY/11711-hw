...
from modules.CLF import CLF, CLF_comb
...
# model_name = 'nerbert'
path = 'data/train1_11_l8_non0_'+model_name+'.npy'
# testpath = 'data/test_'+model_name+'.npy'
# testpath = 'data/test_'+model_name+'.npy'
testpath = 'data/test_submit_'+model_name+'.npy'
...
for epoch in range(start_epoch, epochs):
    ...
    for i, (feats, labels) in enumerate(train_loader):
        ...
        for uttid in range(0, len(labels)):
            pred_save += list(torch.argmax(out[uttid],dim = 1).cpu().numpy())
